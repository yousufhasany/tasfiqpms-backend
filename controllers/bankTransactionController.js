...
/**
 * Recalculate running balance for all bank transactions in a specific project
...
async function recalculateBalances(projectId, startFromDate = null) {
  ...
  if (startFromDate) {
    const prev = await BankTransaction.findOne({
      project: projectId,
  ...
  const txns = await BankTransaction.find(filter).sort({ date: 1, createdAt: 1 });

  let running = openingBalance;
  for (const txn of txns) {
    running = running + Number(txn.credit || 0) - Number(txn.debit || 0);
    await BankTransaction.findByIdAndUpdate(txn._id, { balance: running });
  }
}

// GET /api/bank-transactions?project=<id>
...
// GET /api/bank-transactions/summary?project=<id>
exports.getSummary = async (req, res) => {
  try {
    ...
    const latest = await BankTransaction.findOne(latestFilter).sort({ date: -1, createdAt: -1 });
...
exports.create = async (req, res) => {
  try {
    ...
    const updated = await BankTransaction.findById(txn._id).populate('project', 'name status');
...
// DELETE /api/bank-transactions/:id  (admin only)
